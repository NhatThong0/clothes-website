    ...
    <Link to="/" className="flex items-center gap-2.5 flex-shrink-0">
      <div className="w-9 h-9 rounded-xl overflow-hidden flex-shrink-0">
        {!logoError
          ...
          : <div className="w-full h-full bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center shadow-sm"><span className="text-white font-black text-base">DC</span></div>
        }
      </div>
...
function NotificationBell({ user }) {
  ...
  // Animate bell khi có thông báo mới
  ...
  const handleMarkRead = async (id) => {
    ...
    try { await apiClient.put(`/notifications/${id}/read`); } catch { /* silent */ }
  };
  ...
  const handleMarkAllRead = async () => {
    ...
    try { await apiClient.put('/notifications/read-all'); } catch { /* silent */ }
  };
  ...
  const handleDeleteAll = async () => {
    ...
    setUnreadCount(0);
    prevUnread.current = 0;
    try { await apiClient.delete('/notifications'); } catch { /* silent */ }
  };
            ...
            <div className="flex items-center gap-2">
              ...
              {notifications.length > 0 && (
                <button onClick={handleDeleteAll}
                  className="text-xs font-semibold text-slate-400 hover:text-rose-500 hover:bg-rose-50 px-2 py-1 rounded-lg transition-colors">
                  Xóa tất cả
                </button>
              )}
            </div>
                  ...
                  {items.map(notif => {
                    ...
                    return (
                      <div key={notif._id}
                        ...
                        {/* Icon */}
                        <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-base flex-shrink-0 ${c.bg}`}>
                          {notif.icon}
                        </div>

                        {/* Content */}
                        <div className="flex-1 min-w-0 pr-6">
                          <p className={`text-xs font-bold leading-snug ${notif.isRead ? 'text-slate-700' : 'text-slate-900'}`}>
                            {notif.title}
                          </p>
                        ...
                        {/* Unread dot */}
                        {!notif.isRead && (
                          <div className={`absolute right-9 top-4 w-2 h-2 rounded-full ${c.dot}`} />
                        )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            ...
            {/* 🔔 Notification Bell — chỉ khi đã login */}
          ...
          {NAV_LINKS.map(link => {
            ...
            return (
              <Link key={link.href} to={link.href}
                className={`relative flex items-center gap-1.5 py-3.5 px-4 text-sm font-semibold transition-colors ${active ? 'text-blue-600' : 'text-slate-600 hover:text-blue-600'}`}>
                {link.label}
                {link.badge && (
                  <span className={`text-[10px] font-black text-white px-1.5 py-0.5 rounded-full ${link.badge.cls}`}>{link.badge.label}</span>
                )}
